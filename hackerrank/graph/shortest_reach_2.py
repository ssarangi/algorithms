...
class Node:
    ...
    def update_neighbor(self, node, old_weight, new_weight):
        self.neighbors.remove((node, old_weight))
        self.neighbors.add((node, new_weight))
...
def read(read_fn):
    ...
    for i in range(0, T):
        ...
        for j in range(0, M):
            line = read_fn().split()
            x, y, weight = int(line[0]), int(line[1]), int(line[2])
