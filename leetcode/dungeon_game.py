...
# class Solution(object):
    #def calculateMinimumHP(self, dungeon):
        ...
        # def calculate(dungeon, curr_pt, end_pt, health, min_health):
        #     curr_health = health + dungeon[curr_pt.row][curr_pt.col]
        #     min_health = min(min_health, curr_health)
        #     if curr_pt == end_pt:
        #         return min(curr_health, min_health)

        #     down = sys.maxsize
        #     if curr_pt.row + 1 <= end_pt.row:
        #         down = calculate(dungeon, Room(curr_pt.row + 1, curr_pt.col), end_pt, curr_health, min_health)
        #         down = abs(down)

        #     right = sys.maxsize
        #     if curr_pt.col + 1 <= end_pt.col:
        #         right = calculate(dungeon, Room(curr_pt.row, curr_pt.col + 1), end_pt, curr_health, min_health)
        #         right = abs(right)
        ...
        # health = calculate(dungeon, start_pt, end_pt, 0, 0)
        # return health + 1
...
class Solution(object):
    def calculateMinimumHP(self, dungeon):
        class Node:
            ...
            def __str__(self):
                return "(" + str(self.value) + "," + str(self.min_v) + ")"

            __repr__ = __str__

        dp = [[Node(0, 0) for _ in range(0, len(dungeon[0]))] for _ in range(0, len(dungeon))]

        dp[0][0].value = dungeon[0][0]
        dp[0][0].min_v = dungeon[0][0]

        for col in range(1, len(dungeon[0])):
            dp[0][col].value = dp[0][col - 1].value + dungeon[0][col]
            dp[0][col].min_v = min(dp[0][col - 1].value + dungeon[0][col], dp[0][col - 1].value)

        for row in range(1, len(dungeon)):
            dp[row][0].value = dp[row - 1][0].value + dungeon[row][0]
            dp[row][0].min_v = min(dp[row - 1][0].value + dungeon[row][0], dp[row - 1][0].value)
        ...
        for row in range(1, len(dungeon)):
            for col in range(1, len(dungeon[0])):
                ...
                if above.value < left.value:
                    dp[row][col].value = above.value + dungeon[row][col]
                    dp[row][col].min_v = min(dp[row][col].value, above.min_v)
                else:
                    dp[row][col].value = left.value + dungeon[row][col]
                    dp[row][col].min_v = min(dp[row][col].value, left.min_v)

        end_row = len(dungeon) - 1
        end_col = len(dungeon[0]) - 1

        final_result = dp[end_row][end_col]
        min_health = min(final_result.value, final_result.min_v)
...
class UnitTests(unittest.TestCase):
    ...
    def testIsNumber(self):
        self.assertEqual(self.soln.calculateMinimumHP([[-2, -3, 3], [-5, -10, 1], [10, 30, -5]]), 7)
        self.assertEqual(self.soln.calculateMinimumHP([[-200]]), 201)
        self.assertEqual(self.soln.calculateMinimumHP([[-3, 5]]), 4)
        self.assertEqual(self.soln.calculateMinimumHP([[1,-3,3],[0,-2,0],[-3,-3,-3]]), 3)
        self.assertEqual(self.soln.calculateMinimumHP([[0, 5],[-2,-3]]), 1)
